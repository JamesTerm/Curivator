...
class FRC2015_Robot_Properties : public Tank_Robot_Properties
{
	public:
		...
		const FRC2015_Robot_Props &GetFRC2015RobotProps() const {return m_FRC2015RobotProps;}
		FRC2015_Robot_Props &GetFRC2015RobotProps_rw() {return m_FRC2015RobotProps;}
...
//		const Control_Assignment_Properties &Get_ControlAssignmentProps() const {return m_ControlAssignmentProps;}
	private:
		...
		FRC2015_Robot_Props m_FRC2015RobotProps;
...
const char * const csz_FRC2015_Robot_SpeedControllerDevices_Enum[] =
{
...
const char * const csz_FRC2015_Robot_SolenoidDevices_Enum[] =
{
	"use_low_gear","fork_left","fork_right"
};

const char * const csz_FRC2015_Robot_BoolSensorDevices_Enum[] =
{
	"dart_upper_limit",	"dart_lower_limit"
};

//Note: rotary systems share the same index as their speed controller counterpart
const char * const csz_FRC2015_Robot_AnalogInputs_Enum[] =
{
	"arm_potentiometer"
};


class FRC2015_Robot : public Tank_Robot
{
	public:
		...
		{	return Enum_GetValue<SpeedControllerDevices> (value,csz_FRC2015_Robot_SpeedControllerDevices_Enum,_countof(csz_FRC2015_Robot_SpeedControllerDevices_Enum));
		}
		...
		{	return Enum_GetValue<SolenoidDevices> (value,csz_FRC2015_Robot_SolenoidDevices_Enum,_countof(csz_FRC2015_Robot_SolenoidDevices_Enum));
		}
		...
		{	return Enum_GetValue<BoolSensorDevices> (value,csz_FRC2015_Robot_BoolSensorDevices_Enum,_countof(csz_FRC2015_Robot_BoolSensorDevices_Enum));
		}
		...
		{	return Enum_GetValue<AnalogInputs> (value,csz_FRC2015_Robot_AnalogInputs_Enum,_countof(csz_FRC2015_Robot_AnalogInputs_Enum));
		}

		FRC2015_Robot(const char EntityName[],FRC_2015_Control_Interface *robot_control,bool IsAutonomous=false);
	...
	protected:
		...
		{
			private:
				FRC2015_Robot * const m_pParent;
				double m_Velocity; //adds all axis velocities then assigns on the time change
			public:
				Turret(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control);
				virtual ~Turret() {}
				IEvent::HandlerList ehl;
		...
		class PitchRamp
		{
			private:
				...
				double m_Velocity; 
			public:
				PitchRamp(FRC2015_Robot *pParent,Rotary_Control_Interface *robot_control);
				virtual ~PitchRamp() {}
				IEvent::HandlerList ehl;
				virtual void BindAdditionalEventControls(bool Bind);
				virtual void ResetPos();
	...
	public: //Autonomous public access (wind river has problems with friend technique)
		...
		{
			public:
				Kicker_Wheel(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control);
				IEvent::HandlerList ehl;
			protected:
		...
		{
			public:
				Robot_Arm(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control);
				IEvent::HandlerList ehl;
		...
		const FRC2015_Robot_Properties &GetRobotProps() const;
		FRC2015_Robot_Props::Autonomous_Properties &GetAutonProps();
	...
	private:
		...
		Robot_Arm m_Arm;
		FRC2015_Robot_Properties m_RobotProps;  //saves a copy of all the properties
...
namespace FRC2015_Goals
{
	Goal *Get_FRC2015_Autonomous(FRC2015_Robot *Robot);
}

//TODO work out robot control
#if 0
class FRC2015_Robot_Control : public RobotControlCommon, public FRC_2015_Control_Interface
{
	public:
		FRC2015_Robot_Control(bool UseSafety=true);
		virtual ~FRC2015_Robot_Control();
		...
		const FRC2015_Robot_Properties &GetRobotProps() const {return m_RobotProps;}
...
class FRC2015_Robot_UI : public FRC2015_Robot, public FRC2015_Robot_Control
{
	public:
		FRC2015_Robot_UI(const char EntityName[]);
	protected:
		virtual void TimeChange(double dTime_s);
