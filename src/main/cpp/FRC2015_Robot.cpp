...
static double PositionToVelocity_Tweak(double Value)
{
  ...
  /***********************************************************************************************************************************/
 /*														FRC2015_Robot::Turret														*/
/***********************************************************************************************************************************/

FRC2015_Robot::Turret::Turret(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control) : 	m_pParent(parent),m_Velocity(0.0)
{
}


void FRC2015_Robot::Turret::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=m_pParent->GetEventMap(); //grrr had to explicitly specify which EventMap
	if (Bind)
	{
		em->EventValue_Map["Turret_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC2015_Robot::Turret::Turret_SetRequestedVelocity);
	}
	else
	{
		em->EventValue_Map["Turret_SetCurrentVelocity"].Remove(*this, &FRC2015_Robot::Turret::Turret_SetRequestedVelocity);
	}
}

void FRC2015_Robot::Turret::TimeChange(double dTime_s)
{
	m_Velocity=0.0;
}

void FRC2015_Robot::Turret::ResetPos()
{
	m_Velocity=0.0;
}

  /***********************************************************************************************************************************/
 /*													FRC2015_Robot::PitchRamp														*/
/***********************************************************************************************************************************/
FRC2015_Robot::PitchRamp::PitchRamp(FRC2015_Robot *pParent,Rotary_Control_Interface *robot_control) : m_pParent(pParent),m_Velocity(0.0)
{
}


void FRC2015_Robot::PitchRamp::TimeChange(double dTime_s)
{
	m_Velocity=0.0;
}

void FRC2015_Robot::PitchRamp::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=m_pParent->GetEventMap(); //grrr had to explicitly specify which EventMap
	if (Bind)
	{
		em->EventValue_Map["PitchRamp_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC2015_Robot::PitchRamp::Pitch_SetRequestedVelocity);
	}
	else
	{
		em->EventValue_Map["PitchRamp_SetCurrentVelocity"].Remove(*this, &FRC2015_Robot::PitchRamp::Pitch_SetRequestedVelocity);
	}
}

void FRC2015_Robot::PitchRamp::ResetPos()
{
	m_Velocity=0.0;
}

  /***********************************************************************************************************************************/
 /*													FRC2015_Robot::Kicker_Wheel													*/
/***********************************************************************************************************************************/

FRC2015_Robot::Kicker_Wheel::Kicker_Wheel(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control) :
	Rotary_Velocity_Control("KickerWheel",robot_control,eKickerWheel),m_pParent(parent)
{
}

void FRC2015_Robot::Kicker_Wheel::TimeChange(double dTime_s)
{
...
void FRC2015_Robot::Kicker_Wheel::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=GetEventMap(); //grrr had to explicitly specify which EventMap
	if (Bind)
	{
		em->EventValue_Map["KickerWheel_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC2015_Robot::Kicker_Wheel::Kicker_Wheel_SetRequestedVelocity);
	}
	else
	{
		em->EventValue_Map["KickerWheel_SetCurrentVelocity"].Remove(*this, &FRC2015_Robot::Kicker_Wheel::Kicker_Wheel_SetRequestedVelocity);
	}
}

  /***********************************************************************************************************************************/
 /*													FRC2015_Robot::Robot_Arm														*/
/***********************************************************************************************************************************/

FRC2015_Robot::Robot_Arm::Robot_Arm(FRC2015_Robot *parent,Rotary_Control_Interface *robot_control) : 
Rotary_Position_Control("Arm",robot_control,eArm),m_pParent(parent),m_Advance(false),m_Retract(false)
{
}


void FRC2015_Robot::Robot_Arm::Advance(bool on)
{
	m_Advance=on;
}
void FRC2015_Robot::Robot_Arm::Retract(bool on)
{
	m_Retract=on;
}

bool FRC2015_Robot::Robot_Arm::DidHitMinLimit() const
{
...
bool FRC2015_Robot::Robot_Arm::DidHitMaxLimit() const
{
	#ifndef Robot_TesterCode
...
void FRC2015_Robot::Robot_Arm::TimeChange(double dTime_s)
{
...
#endif
#ifdef Robot_TesterCode
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
...
double FRC2015_Robot::Robot_Arm::AngleToHeight_m(double Angle_r) const
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	const double c_GearToArmRatio=1.0/props.ArmToGearRatio;
...
double FRC2015_Robot::Robot_Arm::Arm_AngleToHeight_m(double Angle_r) const
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	return (sin(Angle_r)*props.ArmLength)+props.GearHeightOffset;
}

double FRC2015_Robot::Robot_Arm::HeightToAngle_r(double Height_m) const
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	return asin((Height_m-props.GearHeightOffset)/props.ArmLength) * props.ArmToGearRatio;
}

double FRC2015_Robot::Robot_Arm::PotentiometerRaw_To_Arm_r(double raw) const
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
...
void FRC2015_Robot::Robot_Arm::SetPosRest()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition(HeightToAngle_r(Inches2Meters(props.ToteRestHeight))  );
}
void FRC2015_Robot::Robot_Arm::SetTote2Height()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition( HeightToAngle_r(Inches2Meters(props.Tote2Height)) );
}
void FRC2015_Robot::Robot_Arm::SetTote3Height()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition(HeightToAngle_r(Inches2Meters(props.Tote3Height)));
}
void FRC2015_Robot::Robot_Arm::SetTote4Height()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition( HeightToAngle_r(Inches2Meters(props.Tote4Height)) );
}
void FRC2015_Robot::Robot_Arm::SetTote5Height()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition( HeightToAngle_r(Inches2Meters(props.Tote5Height)) );
}
void FRC2015_Robot::Robot_Arm::SetTote6Height()
{
	const FRC2015_Robot_Props &props=m_pParent->GetRobotProps().GetFRC2015RobotProps();
	SetIntendedPosition( HeightToAngle_r(Inches2Meters(props.Tote6Height)) );
}

void FRC2015_Robot::Robot_Arm::CloseForkRight(bool Close)
{
	m_pParent->m_RobotControl->CloseSolenoid(eForkRight,Close);
}
void FRC2015_Robot::Robot_Arm::CloseForkLeft(bool Close)
{
	m_pParent->m_RobotControl->CloseSolenoid(eForkLeft,Close);
}

void FRC2015_Robot::Robot_Arm::CloseForkBoth(bool Close)
{
	CloseForkLeft(Close);
	CloseForkRight(Close);
}

void FRC2015_Robot::Robot_Arm::BindAdditionalEventControls(bool Bind)
{
	Base::EventMap *em=GetEventMap(); //grrr had to explicitly specify which EventMap
	if (Bind)
	{
		em->EventValue_Map["Arm_SetCurrentVelocity"].Subscribe(ehl,*this, &FRC2015_Robot::Robot_Arm::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["Arm_SetPotentiometerSafety"].Subscribe(ehl,*this, &FRC2015_Robot::Robot_Arm::SetPotentiometerSafety);

		em->Event_Map["Arm_SetPosRest"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetPosRest);
		em->Event_Map["Arm_SetTote2Height"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetTote2Height);
		em->Event_Map["Arm_SetTote3Height"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetTote3Height);
		em->Event_Map["Arm_SetTote4Height"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetTote4Height);
		em->Event_Map["Arm_SetTote5Height"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetTote5Height);
		em->Event_Map["Arm_SetTote6Height"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::SetTote6Height);

		em->EventOnOff_Map["Arm_Advance"].Subscribe(ehl,*this, &FRC2015_Robot::Robot_Arm::Advance);
		em->EventOnOff_Map["Arm_Retract"].Subscribe(ehl,*this, &FRC2015_Robot::Robot_Arm::Retract);

		em->EventOnOff_Map["Arm_ForkRight"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::CloseForkRight);
		em->EventOnOff_Map["Arm_ForkLeft"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::CloseForkLeft);
		em->EventOnOff_Map["Arm_ForkBoth"].Subscribe(ehl, *this, &FRC2015_Robot::Robot_Arm::CloseForkBoth);
	}
	else
	{
		em->EventValue_Map["Arm_SetCurrentVelocity"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetRequestedVelocity_FromNormalized);
		em->EventOnOff_Map["Arm_SetPotentiometerSafety"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetPotentiometerSafety);

		em->Event_Map["Arm_SetPosRest"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetPosRest);
		em->Event_Map["Arm_SetTote2Height"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetTote2Height);
		em->Event_Map["Arm_SetTote3Height"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetTote3Height);
		em->Event_Map["Arm_SetTote4Height"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetTote4Height);
		em->Event_Map["Arm_SetTote5Height"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetTote5Height);
		em->Event_Map["Arm_SetTote6Height"].Remove(*this, &FRC2015_Robot::Robot_Arm::SetTote6Height);

		em->EventOnOff_Map["Arm_Advance"].Remove(*this, &FRC2015_Robot::Robot_Arm::Advance);
		em->EventOnOff_Map["Arm_Retract"].Remove(*this, &FRC2015_Robot::Robot_Arm::Retract);

		em->EventOnOff_Map["Arm_ForkRight"]  .Remove(*this, &FRC2015_Robot::Robot_Arm::CloseForkRight);
		em->EventOnOff_Map["Arm_ForkLeft"]  .Remove(*this, &FRC2015_Robot::Robot_Arm::CloseForkLeft);
		em->EventOnOff_Map["Arm_ForkBoth"]  .Remove(*this, &FRC2015_Robot::Robot_Arm::CloseForkBoth);
	}
}
...
FRC2015_Robot::FRC2015_Robot(const char EntityName[],FRC_2015_Control_Interface *robot_control,bool IsAutonomous) : 
...
void FRC2015_Robot::Initialize(Entity2D_Kind::EventMap& em, const Entity_Properties *props)
{
	__super::Initialize(em,props);
	m_RobotControl->Initialize(props);

	const FRC2015_Robot_Properties *RobotProps=dynamic_cast<const FRC2015_Robot_Properties *>(props);
	m_RobotProps=*RobotProps;  //Copy all the properties (we'll need them for high and low gearing)

	//set to the default key position
	//const FRC2015_Robot_Props &robot2015props=RobotProps->GetFRC2015RobotProps();
...
void FRC2015_Robot::TimeChange(double dTime_s)
{
	...
	//const FRC2015_Robot_Props &robot_props=m_RobotProps.GetFRC2015RobotProps();
...
const FRC2015_Robot_Properties &FRC2015_Robot::GetRobotProps() const
{
	return m_RobotProps;
}

FRC2015_Robot_Props::Autonomous_Properties &FRC2015_Robot::GetAutonProps()
{
	return m_RobotProps.GetFRC2015RobotProps_rw().Autonomous_Props;
}

void FRC2015_Robot::SetLowGear(bool on) 
{
...
void FRC2015_Robot::SetLowGearValue(double Value)
{
	if (m_IsAutonomous) return;  //We don't want to read joystick settings during autonomous
	//printf("\r%f       ",Value);
	if (Value > 0.0)
...
void FRC2015_Robot::SetDriverOverride(bool on) 
{
	if (m_IsAutonomous) return;  //We don't want to read joystick settings during autonomous
...
void FRC2015_Robot::BindAdditionalEventControls(bool Bind)
{
	Entity2D_Kind::EventMap *em=GetEventMap(); 
	if (Bind)
	{
		em->EventOnOff_Map["Robot_SetLowGear"].Subscribe(ehl, *this, &FRC2015_Robot::SetLowGear);
		em->Event_Map["Robot_SetLowGearOn"].Subscribe(ehl, *this, &FRC2015_Robot::SetLowGearOn);
		em->Event_Map["Robot_SetLowGearOff"].Subscribe(ehl, *this, &FRC2015_Robot::SetLowGearOff);
		em->EventValue_Map["Robot_SetLowGearValue"].Subscribe(ehl,*this, &FRC2015_Robot::SetLowGearValue);
		em->EventOnOff_Map["Robot_SetDriverOverride"].Subscribe(ehl, *this, &FRC2015_Robot::SetDriverOverride);

		//no longer restricted to simulation
		em->Event_Map["TestAuton"].Subscribe(ehl, *this, &FRC2015_Robot::TestAutonomous);
		em->Event_Map["Complete"].Subscribe(ehl,*this,&FRC2015_Robot::GoalComplete);
		em->EventOnOff_Map["StopAuton"].Subscribe(ehl, *this, &FRC2015_Robot::StopAuton);
	}
	else
	{
		em->EventOnOff_Map["Robot_SetLowGear"]  .Remove(*this, &FRC2015_Robot::SetLowGear);
		em->Event_Map["Robot_SetLowGearOn"]  .Remove(*this, &FRC2015_Robot::SetLowGearOn);
		em->Event_Map["Robot_SetLowGearOff"]  .Remove(*this, &FRC2015_Robot::SetLowGearOff);
		em->EventValue_Map["Robot_SetLowGearValue"].Remove(*this, &FRC2015_Robot::SetLowGearValue);
		em->EventOnOff_Map["Robot_SetDriverOverride"]  .Remove(*this, &FRC2015_Robot::SetDriverOverride);

		em->Event_Map["TestAuton"]  .Remove(*this, &FRC2015_Robot::TestAutonomous);
		em->Event_Map["Complete"]  .Remove(*this, &FRC2015_Robot::GoalComplete);
		em->EventOnOff_Map["StopAuton"].Remove(*this, &FRC2015_Robot::StopAuton);
	}
...
void FRC2015_Robot::BindAdditionalUIControls(bool Bind,void *joy, void *key)
{
...
void FRC2015_Robot::UpdateController(double &AuxVelocity,Vec2D &LinearAcceleration,double &AngularAcceleration,bool &LockShipHeadingToOrientation,double dTime_s)
{
...
void FRC2015_Robot::StopAuton(bool isOn)
{
...
void FRC2015_Robot::TestAutonomous()
{
	...
	{
		Goal *goal=NULL;
		goal=FRC2015_Goals::Get_FRC2015_Autonomous(this);
		if (goal)
...
void FRC2015_Robot::GoalComplete()
{
  ...
  /***********************************************************************************************************************************/
 /*													FRC2015_Robot_Properties														*/
/***********************************************************************************************************************************/

const double c_WheelDiameter=Inches2Meters(6);
const double c_MotorToWheelGearRatio=12.0/36.0;

FRC2015_Robot_Properties::FRC2015_Robot_Properties()  : m_TurretProps(
	"Turret",
	2.0,    //Mass
...
{
	{
		...
		FRC2015_Robot_Props::Autonomous_Properties &auton=props.Autonomous_Props;
...
const char *ProcessVec2D(FRC2015_Robot_Props &m_FRC2015RobotProps,Scripting::Script& script,Vec2d &Dest)
{
	const char *err;
	double length, width;	
...
const char *FRC2015_Robot_Properties::ControlEvents::LUA_Controls_GetEvents(size_t index) const
{
	return (index<_countof(g_FRC_2015_Controls_Events))?g_FRC_2015_Controls_Events[index] : NULL;
}
FRC2015_Robot_Properties::ControlEvents FRC2015_Robot_Properties::s_ControlsEvents;

void FRC2015_Robot_Props::Autonomous_Properties::ShowAutonParameters()
{
	if (ShowParameters)
	{
...
void FRC2015_Robot_Properties::LoadFromScript(Scripting::Script& script)
{
	FRC2015_Robot_Props &props=m_FRC2015RobotProps;
...
//	m_ControlAssignmentProps.LoadFromScript(script);
	...
	if (!err) 
	{
		...
		if (!err)
		{
			struct FRC2015_Robot_Props::Autonomous_Properties &auton=m_FRC2015RobotProps.Autonomous_Props;
			{
...
class FRC_2015_Goals_Impl : public AtomicGoal
{
	private:
		...
		{
			FRC2015_Robot *Robot=&Parent->m_Robot;
			FRC2015_Robot::Kicker_Wheel &KickerWheel=Robot->GetKickerWheel();
			//const double PrecisionTolerance=Robot->GetRobotProps().GetTankRobotProps().PrecisionTolerance;
		...
		{
			FRC2015_Robot *Robot=&Parent->m_Robot;
			FRC2015_Robot::Robot_Arm &Arm=Robot->GetArm();
			//const double PrecisionTolerance=Robot->GetRobotProps().GetTankRobotProps().PrecisionTolerance;
			Goal_Ship1D_MoveToPosition *goal_arm=NULL;
			const double position=FRC2015_Robot::Robot_Arm::HeightToAngle_r(&Arm,Inches2Meters(height_in));
		...
		public:
			...
			virtual void Activate()
			{
				const FRC2015_Robot_Props &props=m_Robot.GetRobotProps().GetFRC2015RobotProps();
				AddSubgoal(Move_Straight(m_Parent,-m_AutonProps.FirstMove_ft));
	...
	public:
		FRC_2015_Goals_Impl(FRC2015_Robot &robot) : m_Robot(robot), m_Timer(0.0), 
		...
		void Activate() 
		{
			...
			FRC2015_Robot_Props::Autonomous_Properties &auton=m_Robot.GetAutonProps();
...
#if 0
  /***********************************************************************************************************************************/
 /*													FRC2015_Robot_Control															*/
/***********************************************************************************************************************************/



void FRC2015_Robot_Control::ResetPos()
{
	...
	OpenSolenoid(FRC2015_Robot::eUseLowGear,true);
}

void FRC2015_Robot_Control::UpdateVoltage(size_t index,double Voltage)
{
	switch (index)
	{
	case FRC2015_Robot::eKickerWheel:
...
bool FRC2015_Robot_Control::GetBoolSensorState(size_t index) const
{
	bool ret;
	switch (index)
	{
	case FRC2015_Robot::eDartUpper:
		ret=m_Limit_DartUpper;
		break;
	case FRC2015_Robot::eDartLower:
...
FRC2015_Robot_Control::FRC2015_Robot_Control(bool UseSafety) : m_TankRobotControl(UseSafety),m_pTankRobotControl(&m_TankRobotControl),
		m_Compressor(NULL),m_RoboRIO_Accelerometer(NULL)
{
}

FRC2015_Robot_Control::~FRC2015_Robot_Control()
{
	//Encoder_Stop(FRC2015_Robot::eWinch);
...
void FRC2015_Robot_Control::Reset_Rotary(size_t index)
{
...
#ifdef Robot_TesterCode
void FRC2015_Robot_Control::BindAdditionalEventControls(bool Bind,Base::EventMap *em,IEvent::HandlerList &ehl)
{
}
#endif

void FRC2015_Robot_Control::Initialize(const Entity_Properties *props)
{
	Tank_Drive_Control_Interface *tank_interface=m_pTankRobotControl;
	tank_interface->Initialize(props);

	const FRC2015_Robot_Properties *robot_props=dynamic_cast<const FRC2015_Robot_Properties *>(props);
	...
	if (!m_Compressor)
	{
		...
		//Encoder_SetDistancePerPulse(FRC2015_Robot::eWinch,EncoderPulseRate);
		//Encoder_Start(FRC2015_Robot::eWinch);
...
void FRC2015_Robot_Control::Robot_Control_TimeChange(double dTime_s)
{
	m_Limit_DartUpper=BoolSensor_GetState(FRC2015_Robot::eDartUpper);
	SmartDashboard::PutBoolean("LimitDartUpper",m_Limit_DartUpper);
	m_Limit_DartLower=BoolSensor_GetState(FRC2015_Robot::eDartLower);
...
void FRC2015_Robot_Control::UpdateLeftRightVoltage(double LeftVoltage,double RightVoltage) 
{
	...
	{
		Victor_UpdateVoltage(FRC2015_Robot::eLeftDrive3,(float)LeftVoltage * TankRobotProps.VoltageScalar_Left);
		Victor_UpdateVoltage(FRC2015_Robot::eRightDrive3,-(float)RightVoltage * TankRobotProps.VoltageScalar_Right);
	}
	else
	{
		Victor_UpdateVoltage(FRC2015_Robot::eLeftDrive3,(float)RightVoltage * TankRobotProps.VoltageScalar_Right);
		Victor_UpdateVoltage(FRC2015_Robot::eRightDrive3,-(float)LeftVoltage * TankRobotProps.VoltageScalar_Left);
	}
	#endif
	m_pTankRobotControl->UpdateLeftRightVoltage(LeftVoltage,RightVoltage);
}

double FRC2015_Robot_Control::GetRotaryCurrentPorV(size_t index)
{
	double result=0.0;
	const FRC2015_Robot_Props &props=m_RobotProps.GetFRC2015RobotProps();

	switch (index)
	{
		case FRC2015_Robot::eArmPotentiometer:
		{
			#ifndef Robot_TesterCode
			//double raw_value = (double)m_Potentiometer.GetAverageValue();
			double raw_value=(double)Analog_GetAverageValue(FRC2015_Robot::eArmPotentiometer);
...
void FRC2015_Robot_Control::OpenSolenoid(size_t index,bool Open)
{
	switch (index)
	{
	case FRC2015_Robot::eUseLowGear:
		SmartDashboard::PutBoolean("UseHighGear",!Open);
		Solenoid_Open(index,Open);
		break;
	case FRC2015_Robot::eForkLeft:
		SmartDashboard::PutBoolean("ForkLeft",!Open);
		Solenoid_Open(index,Open);
		break;
	case FRC2015_Robot::eForkRight:
...
FRC2015_Robot_UI::FRC2015_Robot_UI(const char EntityName[]) : FRC2015_Robot(EntityName,this),FRC2015_Robot_Control(),
		m_TankUI(this)
{
}

void FRC2015_Robot_UI::TimeChange(double dTime_s) 
{
	__super::TimeChange(dTime_s);
	m_TankUI.TimeChange(dTime_s);
}
void FRC2015_Robot_UI::Initialize(Entity2D::EventMap& em, const Entity_Properties *props)
{
	__super::Initialize(em,props);
	m_TankUI.Initialize(em,props);
}

void FRC2015_Robot_UI::UI_Init(Actor_Text *parent) 
{
	m_TankUI.UI_Init(parent);
}
void FRC2015_Robot_UI::custom_update(osg::NodeVisitor *nv, osg::Drawable *draw,const osg::Vec3 &parent_pos) 
{
	m_TankUI.custom_update(nv,draw,parent_pos);
}
void FRC2015_Robot_UI::Text_SizeToUse(double SizeToUse) 
{
	m_TankUI.Text_SizeToUse(SizeToUse);
}
void FRC2015_Robot_UI::UpdateScene (osg::Geode *geode, bool AddOrRemove) 
{
